...
struct ImDrawChannel;               // Temporary storage to output drawInspector commands out of order, used by ImDrawListSplitter and ImDrawList::ChannelsSplit()
struct ImDrawCmd;                   // A single drawInspector command within a parent ImDrawList (generally maps to 1 GPU drawInspector call, unless it is a callback)
struct ImDrawData;                  // All drawInspector command lists required to render the frame + pos/size coordinates to use for the projection matrix.
struct ImDrawList;                  // A single drawInspector command list (generally one per window, conceptually you may see this as a dynamic "mesh" builder)
struct ImDrawListSharedData;        // Data shared among multiple drawInspector lists (typically owned by parent ImGui context, but you may create one yourself)
struct ImDrawListSplitter;          // Helper to split a drawInspector list into different layers which can be drawn into out of order, then flattened back.
...
namespace ImGui
{
    ...
    IMGUI_API void          Render();                                   // ends the Dear ImGui frame, finalize the drawInspector data. You can then get call GetDrawData().
    IMGUI_API ImDrawData*   GetDrawData();                              // valid after Render() and until the next call to NewFrame(). this is what you have to render.

    // Demo, Debug, Information
    IMGUI_API void          ShowDemoWindow(bool* p_open = NULL);        // create Demo window. demonstrate most ImGui features. call this to learn about the library! try to make it always available in your application!
    IMGUI_API void          ShowMetricsWindow(bool* p_open = NULL);     // create Metrics/Debugger window. display Dear ImGui internals: windows, drawInspector commands, various internal state, etc.
    ...
    IMGUI_API ImDrawList*   GetWindowDrawList();                        // get drawInspector list associated to the current window, to append your own drawing primitives
    ...
    IMGUI_API ImVec2        GetFontTexUvWhitePixel();                                       // get UV coordinate for a while pixel, useful to drawInspector custom shapes via the ImDrawList API
    ...
    IMGUI_API void          Bullet();                                                       // drawInspector a small circle + keep the cursor on the same line. advance cursor x position by GetTreeNodeToLabelSpacing(), same distance that TreeNode() uses
    ...
    IMGUI_API ImDrawList*   GetBackgroundDrawList();                                            // this drawInspector list will be the first rendering one. Useful to quickly drawInspector shapes/text behind dear imgui contents.
    IMGUI_API ImDrawList*   GetForegroundDrawList();                                            // this drawInspector list will be the last rendered one. Useful to quickly drawInspector shapes/text over dear imgui contents.
...
enum ImGuiTableFlags_
{
    ...
    // Clipping
    ImGuiTableFlags_NoClip                     = 1 << 20,  // Disable clipping rectangle for every individual columns (reduce drawInspector command count, items will be able to overflow into other columns). Generally incompatible with TableSetupScrollFreeze().
    // Padding
...
enum ImGuiTableColumnFlags_
{
    ...
    ImGuiTableColumnFlags_NoClip                = 1 << 8,   // Disable clipping for this column (all NoClip columns will render in a same drawInspector command).
...
//  - Layer 0: drawInspector with RowBg0 color if set, otherwise drawInspector with ColumnBg0 if set.
//  - Layer 1: drawInspector with RowBg1 color if set, otherwise drawInspector with ColumnBg1 if set.
//  - Layer 2: drawInspector with CellBg color if set.
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_AcceptNoDrawDefaultRect      = 1 << 11,  // Do not drawInspector the default highlight rectangle when hovering over target.
...
struct ImGuiIO
{
    ...
    bool        MouseDrawCursor;                // = false          // Request ImGui to drawInspector a mouse cursor for you (if you are on a platform without a mouse cursor). Cannot be easily renamed to 'io.ConfigXXX' because this is frequently used by backend implementations.
...
struct ImGuiListClipper
{
    ...
    IMGUI_API bool  Step();            // Call until it returns false. The DisplayStart/DisplayEnd fields will be set and you can process/drawInspector those items.
...
// NB: You most likely do NOT need to use drawInspector callbacks just to create your own widget or customized UI rendering,
// you can poke into the drawInspector list for that! Draw callback may be useful for example to:
...
// Typically, 1 command = 1 GPU drawInspector call (unless command is a callback)
...
struct ImDrawCmd
{
    ...
    void*           UserCallbackData;   // 4-8  // The drawInspector callback code can access this.

    ImDrawCmd() { memset(this, 0, sizeof(*this)); } // Also ensure our padding fields are zeroed

    // Since 1.83: returns ImTextureID associated with this drawInspector call. Warning: DO NOT assume this is always same as 'TextureId' (we will change this function for an upcoming feature)
...
// Split/Merge functions are used to split the drawInspector list into different layers which can be drawn into out of order.
// This is used by the Columns/Tables API, so items of each column can be batched together in a same drawInspector call.
...
// access the current window drawInspector list and drawInspector custom primitives.
// You can interleave normal ImGui:: calls and adding primitives to the current drawInspector list.
...
struct ImDrawList
{
    ...
    ImVector<ImDrawCmd>     CmdBuffer;          // Draw commands. Typically 1 command = 1 GPU drawInspector call, unless the command is a callback.
    ...
    const ImDrawListSharedData* _Data;          // Pointer to shared drawInspector data (you can use ImGui::GetDrawListSharedData() to get the one from current ImGui context)
    ...
    IMGUI_API void  AddDrawCmd();                                               // This is useful if you need to forcefully create a new drawInspector call (to allow for dependent rendering / blending). Otherwise primitives are merged into the same drawInspector-call as much as possible
    IMGUI_API ImDrawList* CloneOutput() const;                                  // Create a clone of the CmdBuffer/IdxBuffer/VtxBuffer.

    // Advanced: Channels
    // - Use to split render into layers. By switching channels to can render out-of-order (e.g. submit FG primitives before BG primitives)
    // - Use to minimize drawInspector calls (e.g. if going back-and-forth between multiple clipping rectangles, prefer to append into separate channels then merge at the end)
...
// All drawInspector data to render a Dear ImGui frame
