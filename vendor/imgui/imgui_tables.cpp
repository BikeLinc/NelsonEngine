...
//    | TableDrawContextMenu()                  - drawInspector right-click context menu
//-----------------------------------------------------------------------------
...
//    | TableDrawBorders()                      - drawInspector outer borders, inner vertical borders
//    | TableMergeDrawChannels()                - merge drawInspector channels if clipping isn't required
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        // - ClipRect.Min.x: Because merging drawInspector commands doesn't compare min boundaries, we make ClipRect.Min.x match left bounds to be consistent regardless of merging.
    ...
    // [Part 9] Allocate drawInspector channels and setup background cliprect
...
void    ImGui::EndTable()
{
...
#if 0
    // Strip out dummy channel drawInspector calls
    // We have no way to prevent user submitting direct ImDrawList calls into a hidden column (but ImGui:: calls will be clipped out)
    // Pros: remove drawInspector calls which will have no effect. since they'll have zero-size cliprect they may be early out anyway.
    ...
    // Flatten channels and merge drawInspector calls
...
void ImGui::TableSetBgColor(ImGuiTableBgTarget target, ImU32 color, int column_n)
{
    ...
    // We cannot drawInspector neither the cell or row background immediately as we don't know the row height at this point in time.
    switch (target)
    {
...
void ImGui::TableBeginCell(ImGuiTable* table, int column_n)
{
    ...
    {
        // FIXME-TABLE: Could avoid this if drawInspector channel is dummy channel?
...
// Allocate drawInspector channels. Called by TableUpdateLayout()
// - We allocate them following storage order instead of display order so reordering columns won't needlessly
//   increase overall dormant memory cost.
// - We isolate headers drawInspector commands in their own channels instead of just altering clip rects.
//   This is in order to facilitate merging of drawInspector commands.
// - After crossing FreezeRowsCount, all columns see their current drawInspector channel changed to a second set of channels.
// - We only use the dummy drawInspector channel so we can push a null clipping rectangle into it without affecting other
//   channels, while simplifying per-row/per-cell overhead. It will be empty and discarded when merged.
// - We allocate 1 or 2 background drawInspector channels. This is because we know TablePushBackgroundChannel() is only used for
//   horizontal spanning. If we allowed vertical spanning we'd need one background drawInspector channel per merge group (1-4).
// Draw channel allocation (before merging):
// - NoClip                       --> 2+D+1 channels: bg0/1 + bg2 + foreground (same clip rect == always 1 drawInspector call)
// - Clip                         --> 2+D+N channels
...
void ImGui::TableSetupDrawChannels(ImGuiTable* table)
{
    ...
    // Initial drawInspector cmd starts with a BgClipRect that matches the one of its host, to facilitate merge drawInspector commands by default.
    // All our cell highlight are manually clipped with BgClipRect. When unfreezing it will be made smaller to fit scrolling rect.
    // (This technically isn't part of setting up drawInspector channels, but is reasonably related to be done here)
...
// This function reorder drawInspector channels based on matching clip rectangle, to facilitate merging them. Called by EndTable().
...
// We reorder drawInspector commands by arranging them into a maximum of 4 distinct groups:
//
...
// This function assume that each column are pointing to a distinct drawInspector channel,
...
// - The channel uses more than one drawInspector command itself. We drop all our attempt at merging stuff here..
//   we could do better but it's going to be rare and probably not worth the hassle.
// Columns for which the drawInspector channel(s) haven't been merged with other will use their own ImDrawCmd.
//
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        for (int merge_group_sub_n = 0; merge_group_sub_n < merge_group_sub_count; merge_group_sub_n++)
        {
            ...
            // Don't attempt to merge if there are multiple drawInspector calls within the column
        ...
        // Invalidate current drawInspector channel
    ...
    if (merge_group_mask != 0)
    {
        ...
        for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            if (int merge_channels_count = merge_groups[merge_group_n].ChannelsCount)
            {
                ...
                // Extend outer-most clip limits to match those of host, so drawInspector calls can be merged even if
...
void ImGui::TableDrawBorders(ImGuiTable* table)
{
    ...
    if (table->Flags & ImGuiTableFlags_BordersInnerV)
    {
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            // Always drawInspector full height border when being resized/hovered, or on the delimitation of frozen column scrolling.
            ImU32 col;
            float draw_y2;
    ...
    {
        // Display outer border offset by 1 which is a simple way to display it without adding an extra drawInspector call
...
// We cpu-clip text here so that all columns headers can be merged into a same drawInspector call.
...
void ImGui::TableHeader(const char* label)
{
    ...
    // be merged into a single drawInspector call.
...
// Get into the columns background drawInspector command (which is generally the same drawInspector command as before we called BeginColumns)
