...
{
    int                 DrawListsLastFrame[2];  // Last frame number the background (0) and foreground (1) drawInspector lists were used
    ImDrawList*         DrawLists[2];           // Convenience background (0) and foreground (1) drawInspector lists. We use them to drawInspector software mouser cursor when io.MouseDrawCursor is set and to drawInspector most debug overlays.
...
struct ImGuiTableColumn
{
    ...
    float                   ContentMaxXHeadersUsed;         // Contents maximum position for headers rows (regardless of freezing). TableHeader() automatically softclip itself + report ideal desired size, to avoid creating extraneous drawInspector calls
...
struct IMGUI_API ImGuiTable
{
    ...
    ImRect                      HostClipRect;               // This is used to check if we can eventually merge our columns drawInspector calls into the current drawInspector call of the current window.
    ImRect                      HostBackupInnerClipRect;    // Backup of InnerWindow->ClipRect during PushTableBackground()/PopTableBackground()
    ImGuiWindow*                OuterWindow;                // Parent window for the table
    ImGuiWindow*                InnerWindow;                // Window holding the table data (== OuterWindow or a child window)
    ImGuiTextBuffer             ColumnsNames;               // Contiguous buffer holding columns names
    ImDrawListSplitter*         DrawSplitter;               // Shortcut to TempData->DrawSplitter while in table. Isolate drawInspector commands per columns to avoid switching clip rect constantly
...
namespace ImGui
{
    ...
    IMGUI_API ImDrawList*   GetBackgroundDrawList(ImGuiViewport* viewport);                     // get background drawInspector list for the given viewport. this drawInspector list will be the first rendering one. Useful to quickly drawInspector shapes/text behind dear imgui contents.
    IMGUI_API ImDrawList*   GetForegroundDrawList(ImGuiViewport* viewport);                     // get foreground drawInspector list for the given viewport. this drawInspector list will be the last rendered one. Useful to quickly drawInspector shapes/text over dear imgui contents.
