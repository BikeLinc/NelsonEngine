...
struct Model {
	...
	bool LoadOBJ(const char* objPath, const char* mtlBaseDir) {
		...
		for (const auto& shape : shapes) {
			...
			for (size_t f = 0; f < shape.mesh.num_face_vertices.size(); f++) {
				...
				for (size_t v = 0; v < 3; v++) {
					...
					const unsigned int nextIndex = static_cast<unsigned int>(geometry.vertices.size() / 5);
					geometry.vertices.push_back(vx);
					geometry.vertices.push_back(vy);
					geometry.vertices.push_back(vz);
					geometry.vertices.push_back(tx);
					geometry.vertices.push_back(ty);
		...
		for (const auto& entry : geometryByMaterial) {
			...
			totalVertices += geometry.vertices.size() / 5;
	...
	void draw(Transform offset, const glm::vec4& tint = glm::vec4(1.0f), bool wireframe = false) {
		...
		if (std::isnan(offset.position.x) || std::isnan(offset.position.y) || std::isnan(offset.position.z)) return;
		if (std::isnan(offset.scale.x) || std::isnan(offset.scale.y) || std::isnan(offset.scale.z)) return;
		if (offset.scale.x <= 0.0f || offset.scale.y <= 0.0f || offset.scale.z <= 0.0f) return;
